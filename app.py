...
def build_output_workbook(opening_assets: pd.DataFrame, additions: pd.DataFrame, disposals: pd.DataFrame,
                          detail_df: pd.DataFrame, summary_df: pd.DataFrame, scrap_summary_df: pd.DataFrame) -> bytes:
    wb = Workbook()
    ws0 = wb.active
    ws0.title = 'README'
    notes = [
...
if base_file is not None:
    try:
        ...
        output_file = build_output_workbook(opening_assets, additions, disposals, detail_df, summary_df, scrap_summary_df)
