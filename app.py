...
def ensure_series(df, col, default=0):
...
def calculate_fy_2026_27(df: pd.DataFrame, class_life_overrides: dict):
    ...
    df["closing_acc_dep_fy26_27"] = (ensure_series(df, "opening_acc_dep", 0).fillna(0) + df["depreciation_fy26_27"]).round(2)
...
def summary_table(df: pd.DataFrame):
    ...
    if group_col is None:
        return pd.DataFrame([{
            ...
            "Opening Acc Dep": ensure_series(df, "opening_acc_dep", 0).fillna(0).sum(),
