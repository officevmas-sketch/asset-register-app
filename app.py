...
BASE_REQUIRED_COLS = {
    ...
    'Put To Use Date': ['opening assets dates', 'put to use date', 'put-to-use date', 'capitalisation date', 'depreciation start date'],
...
ASSET_CLASS_OPTIONS = [
    'AIRC', 'CAME', 'COMP', 'ELFI', 'FRFX', 'IESO', 'OEQP', 'VEHL',
    'ICBO', 'ICQB', 'ICVI', 'ITPR', 'IUMS', 'IDSO', 'LEHI'
]

INTANGIBLE_ASSET_CLASSES = {'IESO', 'ICBO', 'ICVI', 'ITPR', 'IUMS', 'IDSO', 'ICQB'}
...
def read_uploaded_workbook(uploaded_file) -> Tuple[pd.DataFrame, str, pd.DataFrame]:
    ...
    # Prefer the latest FY 2025-26 detailed register as the opening base for FY 2026-27.
    # The uploaded workbook may contain several historical sheets; selecting the first FY sheet can
    # accidentally pick FY 2024-25 and distort depreciation.
    preferred_patterns = [
        'fy 25 26 new', 'fy-25-26-new', 'fy25 26 new',
        'fy 25 26', 'fy-25-26', 'fy25 26',
        'asset register', 'asset'
    ]
    detail_sheet = None
    normalized_names = {sheet: normalize_header(sheet).replace('-', ' ') for sheet in sheet_names}
    for pattern in preferred_patterns:
        normalized_pattern = normalize_header(pattern).replace('-', ' ')
        for sheet, n in normalized_names.items():
            if normalized_pattern in n:
                detail_sheet = sheet
                break
...
def prepare_opening_assets(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'Put To Use Date' not in df.columns:
        df['Put To Use Date'] = df['Purchase Date']
    ...
    df['Put To Use Date'] = coerce_date(df['Put To Use Date']).fillna(df['Purchase Date'])
    ...
    return df[[
        'Assets ID', 'Assets Description', 'Assets Class', 'Assets Location', 'Purchase Date', 'Put To Use Date',
...
def is_tangible_asset(asset_class: object) -> bool:
    ...
    if value == '':
        return True
    return value not in INTANGIBLE_ASSET_CLASSES
...
def compute_monthly_depreciation(start_date: pd.Timestamp,
    ...
    """Compute depreciation using the same concept as the source workbook.

    Daily depreciation is based on the normal depreciable amount over the full useful
    life, from Put-to-Use / Opening Assets Date to Expiry Date. It is not re-spread
    over the remaining FY period. Posting stops at the earlier of FY end, disposal
    date, or expiry date.
    """
    monthly = {label: 0.0 for label in MONTH_LABELS}
    gross = max(float(gross_block or 0.0), 0.0)
    opening_accum = max(float(opening_accum_dep or 0.0), 0.0)
    book_value = max(gross - opening_accum, 0.0)
    floor = float(scrap_floor or 0.0) if cap_at_scrap else 0.0

    if pd.isna(put_to_use_date):
        put_to_use_date = start_date if pd.notna(start_date) else FY_START
    if pd.isna(start_date):
        start_date = put_to_use_date

    calc_start = max(FY_START, start_date, put_to_use_date)
    useful_life_end = remaining_life_end if pd.notna(remaining_life_end) else FY_END
    posting_end = min(FY_END, end_date if pd.notna(end_date) else FY_END, useful_life_end)

    if book_value <= floor + 0.01:
        status = 'At Scrap value' if cap_at_scrap else 'No Depreciation'
        return monthly, 0.0, status
    if posting_end < calc_start:
        status = 'Expired - No Depreciation' if useful_life_end < calc_start else 'No Depreciation'
        return monthly, 0.0, status

    # Excel equivalent: life days = Expiry Date - Put-to-Use Date, and
    # depreciation per day = (Gross Block - Scrap Value) / Life Days.
    total_life_days = max(int((useful_life_end - put_to_use_date).days), 1)
    depreciable_base = max(gross - floor, 0.0)
    per_day = depreciable_base / total_life_days if total_life_days else 0.0
    ...
    for month_end, label in zip(MONTH_ENDS, MONTH_LABELS):
        ...
        if active_end >= active_start:
            ...
            if cap_at_scrap:
                ...
                dep = min(dep, max(current_value, 0.0))
...
def process_assets(opening_assets: pd.DataFrame, additions: pd.DataFrame, disposals: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in opening_assets.iterrows():
        ...
        put_to_use_date = pd.to_datetime(row.get('Put To Use Date', purchase_date))
        if pd.isna(put_to_use_date):
            put_to_use_date = purchase_date
        life_years = float(row['Assets Life']) if not pd.isna(row['Assets Life']) else 5.0
        life_end = put_to_use_date + pd.DateOffset(years=int(life_years))
        ...
        monthly, total_dep, status = compute_monthly_depreciation(
            start_date=max(FY_START, put_to_use_date),
            end_date=dep_end,
            gross_block=opening_gross,
    ...
    for item in processed:
        row = {
            ...
            'Put To Use Date': item.put_to_use_date,
...
def build_output_workbook(opening_assets: pd.DataFrame, additions: pd.DataFrame, disposals: pd.DataFrame,
    ...
    notes = [
        ...
        ['2. Depreciation is calculated basis the put-to-use date / Opening Assets Date from the source register. Additions are treated as put to use from the addition date.'],
        ['3. Sold assets are depreciated up to the disposal date and net block becomes zero after sale.'],
        ['4. Depreciation is calculated only up to the earlier of FY end, disposal date, or asset expiry date.'],
        ['5. Where Cap At Scrap Value = Yes, closing net block is not allowed to go below salvage value for assets not sold.'],
        ['6. Profit / (loss) on sale is computed as Sale Value - (Gross Block - Accumulated Depreciation up to sale date).'],
        ['7. Disposal processing is currently designed as full asset sale at asset-ID level.'],
...
def build_additions_template_xlsx() -> bytes:
    ...
    asset_classes = list(ASSET_CLASS_OPTIONS)
    ...
    for row in range(2, max_rows + 2):
        ...
        ws[f'G{row}'] = f'=IF(OR(D{row}="",F{row}=""),"",IF(OR(D{row}="IESO",D{row}="ICBO",D{row}="ICVI",D{row}="ITPR",D{row}="IUMS",D{row}="IDSO",D{row}="ICQB"),0,ROUND(F{row}*5%,2)))'
        ws[f'K{row}'] = f'=IF(D{row}="","",IF(OR(D{row}="IESO",D{row}="ICBO",D{row}="ICVI",D{row}="ITPR",D{row}="IUMS",D{row}="IDSO",D{row}="ICQB"),"No","Yes"))'
...
def build_disposals_template_xlsx(asset_master: Optional[pd.DataFrame] = None) -> bytes:
    ...
    if asset_master is not None and not asset_master.empty:
        ...
        lookup['Assets ID'] = lookup['Assets ID'].astype(str).str.strip()
        lookup['Opening Gross FY26_27'] = pd.to_numeric(lookup['Opening Gross FY26_27'], errors='coerce').fillna(0.0)
        lookup = lookup[lookup['Assets ID'].ne('') & lookup['Assets ID'].ne('nan')]
        ...
        for i, row in enumerate(lookup.itertuples(index=False), start=2):
            list_ws[f'A{i}'] = row[0]
            list_ws[f'B{i}'] = float(row[1])

        max_lookup_row = max(len(lookup) + 1, 2)
        asset_lookup_ref = f"'Asset_Lookup'!$A$2:$B${max_lookup_row}"
        try:
            wb.defined_names.add(DefinedName('AssetLookupTable', attr_text=asset_lookup_ref))
        except Exception:
            wb.defined_names.append(DefinedName('AssetLookupTable', attr_text=asset_lookup_ref))

        dv_assets = DataValidation(type='list', formula1=f'=Asset_Lookup!$A$2:$A${max_lookup_row}', allow_blank=True)
        ws.add_data_validation(dv_assets)

        for row in range(2, 502):
            dv_assets.add(ws[f'B{row}'])
            ws[f'C{row}'] = f'=IF(B{row}="","",IFERROR(VLOOKUP(B{row}&"",AssetLookupTable,2,FALSE),""))'
...
with st.sidebar:
    ...
    st.info('Additions template includes the requested asset class dropdown, plus depreciation method, cap at scrap value, and asset life. Depreciation rate is a manual-entry column. Tangible classes default to 5% salvage, while the specified intangible classes default to zero salvage and no scrap cap. After the base register is uploaded, the in-app disposal template includes an Asset ID dropdown and auto-pulls Gross Block into Disposed Amount.')
...
if base_file is not None:
    try:
        ...
        st.error(f'Unable to read the base asset register: {exc}')
        st.stop()
    ...
    try:
        ...
        st.caption('This download is available immediately after the base register is uploaded. Select the Asset ID in Excel and Disposed Amount will auto-pull from Gross Block.')
    except Exception as exc:
        st.warning(f'Unable to prepare the disposal template: {exc}')
    ...
    try:
        ...
        st.error(f'Unable to process the FY 2026-27 computation: {exc}')
else:
    st.markdown('''
    **What this app does**
    ...
    - uses Opening Assets Dates / put-to-use date for depreciation expiry and start logic
