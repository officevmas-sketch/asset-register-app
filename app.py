...
st.set_page_config(page_title="Asset Register Automation", layout="wide")
...
def calculate_fy_2026_27(df: pd.DataFrame, class_life_overrides: dict):
    ...
    df["days_in_service_fy26_27"] = df.apply(
        lambda r: days_in_service_for_fy(r.get("purchase_date"), r.get("disposal_date") if "disposed" in r["status_norm"] else pd.NaT),
        axis=1,
    )
