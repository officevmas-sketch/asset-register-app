...
DISPOSAL_TEMPLATE_COLS = ['Disposal Date', 'Assets ID', 'Disposed Amount', 'Remark']
...
def prepare_disposals(disposals_file) -> pd.DataFrame:
    ...
    for col in DISPOSAL_TEMPLATE_COLS:
        if col not in df.columns:
            df[col] = '' if col not in ['Disposed Amount'] else 0
...
def process_assets(opening_assets: pd.DataFrame, additions: pd.DataFrame, disposals: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in opening_assets.iterrows():
        ...
        if pd.notna(disposal_date):
            ...
            closing_accum = max(round(opening_accum + total_dep - dep_on_disposal, 2), 0.0)
            closing_gross = max(round(opening_gross - deletion_amount, 2), 0.0)
            closing_nb = max(round(closing_gross - closing_accum, 2), 0.0)
            status = 'Disposed'
        else:
    ...
    if not additions.empty:
        for _, row in additions.iterrows():
            ...
            if pd.notna(disposal_date):
                ...
                closing_accum = max(round(total_dep - dep_on_disposal, 2), 0.0)
                closing_gross = max(round(cost - deletion_amount, 2), 0.0)
                closing_nb = max(round(closing_gross - closing_accum, 2), 0.0)
...
def build_summary(detail_df: pd.DataFrame) -> pd.DataFrame:
    sum_cols = [
        ...
        *MONTH_LABELS,
        'Total Depreciation FY 2026-27', 'Depreciation on Disposal',
...
def build_output_workbook(opening_assets: pd.DataFrame, additions: pd.DataFrame, disposals: pd.DataFrame,
                          detail_df: pd.DataFrame, summary_df: pd.DataFrame) -> bytes:
    wb = Workbook()
    ws0 = wb.active
    ws0.title = 'README'
    notes = [
        ...
        ['5. Review the detailed register and summary before finalisation.'],
    ]
    for row in notes:
        ws0.append(row)
...
with st.sidebar:
    ...
    st.info('Scrap value floor works where `Cap At Scrap Value` is Yes.')
...
if base_file is not None:
    try:
        ...
        output_file = build_output_workbook(opening_assets, additions, disposals, detail_df, summary_df)
        ...
        st.dataframe(detail_df.head(300), use_container_width=True, height=500)
    ...
    **What this app does**
    ...
    - gives a detailed register and a class-wise summary workbook for download
    ''')
