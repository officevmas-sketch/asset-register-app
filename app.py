...
def coerce_numeric(series, fill=0.0, index=None):
    if isinstance(series, pd.Series):
        return pd.to_numeric(series, errors="coerce").fillna(fill)
    if series is None or np.isscalar(series):
        if index is None:
            return pd.Series([fill], dtype="float64")
        return pd.Series([fill] * len(index), index=index, dtype="float64")
    converted = pd.to_numeric(pd.Series(series, index=index), errors="coerce")
    return converted.fillna(fill)
...
def month_ends_for_fy(start=FY_START):
    months = pd.date_range(start, FY_END, freq="ME")
    return list(months)
...
def prepare_base_dataframe(df, cols):
    ...
    out["description"] = out.get("description", pd.Series([""] * len(df), index=df.index, dtype="object"))
    out["asset_class"] = out.get("asset_class", pd.Series([""] * len(df), index=df.index, dtype="object"))
    out["nature"] = out.get("nature", pd.Series([""] * len(df), index=df.index, dtype="object"))
    out["location"] = out.get("location", pd.Series([""] * len(df), index=df.index, dtype="object"))
    out["vendor"] = out.get("vendor", pd.Series([""] * len(df), index=df.index, dtype="object"))
    out["purchase_date"] = coerce_date(out.get("purchase_date", pd.Series([pd.NaT] * len(df), index=df.index)))
    out["status"] = out.get("status", pd.Series([""] * len(df), index=df.index, dtype="object")).apply(safe_status_text)
    out["gross_closing"] = coerce_numeric(out.get("gross_closing", 0), index=df.index)
    out["closing_gross"] = coerce_numeric(out.get("closing_gross", out["gross_closing"]), index=df.index)
    out["salvage"] = coerce_numeric(out.get("salvage", 0), index=df.index)
    out["asset_life"] = coerce_numeric(out.get("asset_life", 0), index=df.index)
    out["life_days"] = coerce_numeric(out.get("life_days", 0), index=df.index)
    out["opening_acc_dep"] = coerce_numeric(out.get("closing_acc_dep", out.get("opening_acc_dep", 0)), index=df.index)
    out["opening_net"] = coerce_numeric(out.get("closing_net", out.get("opening_net", 0)), index=df.index)
    out["sub_classification"] = out.get("sub_classification", pd.Series([""] * len(df), index=df.index, dtype="object"))
    out["remark"] = out.get("remark", pd.Series([""] * len(df), index=df.index, dtype="object"))
    return out
