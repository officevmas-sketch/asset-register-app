...
def process_assets(opening_assets: pd.DataFrame, additions: pd.DataFrame, disposals: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in opening_assets.iterrows():
        ...
        if pd.notna(disposal_date):
            dep_on_disposal = round(opening_accum + total_dep, 2)
            carrying_value_on_sale = max(round(deletion_amount - dep_on_disposal, 2), 0.0)
        ...
        processed.append(ProcessedAsset(
            ...
            profit_on_sale=round(max(profit_loss_on_sale, 0.0), 2),
            loss_on_sale=round(max(-profit_loss_on_sale, 0.0), 2),
        ))
...
def build_summary(detail_df: pd.DataFrame) -> pd.DataFrame:
    sum_cols = [
        ...
        *MONTH_LABELS,
        'Total Depreciation FY 2026-27', 'Depreciation on Disposal', 'Sale Value on Disposal', 'Carrying Value on Sale', 'Profit / (Loss) on Sale', 'Profit on Sale', 'Loss on Sale',
...
def build_disposal_summary(detail_df: pd.DataFrame) -> pd.DataFrame:
    ...
    cols = [
        ...
        'Carrying Value on Sale', 'Sale Value on Disposal', 'Profit / (Loss) on Sale', 'Profit on Sale', 'Loss on Sale', 'Status'
    ]
    ...
    total_row = {col: '' for col in cols}
    total_row['Assets ID'] = 'TOTAL'
    for col in ['Deletion During FY 2026-27', 'Depreciation on Disposal', 'Carrying Value on Sale', 'Sale Value on Disposal', 'Profit / (Loss) on Sale', 'Profit on Sale', 'Loss on Sale']:
...
def build_output_workbook(opening_assets: pd.DataFrame, additions: pd.DataFrame, disposals: pd.DataFrame,
    ...
    notes = [
        ...
        ['5. For disposed assets, closing net block is forced to zero in the FY-end register.'],
        ['6. Profit / (loss) on sale is computed as Sale Value minus Carrying Value on Sale.'],
...
with st.sidebar:
    ...
    st.info('Scrap value floor works where `Cap At Scrap Value` is Yes. Disposal template now also supports `Sale Value` for profit / loss on sale.')
...
if base_file is not None:
    try:
        ...
        k8.metric('Profit on sale', f"{total_profit_on_sale:,.2f}")
        k9.metric('Loss on sale', f"{total_loss_on_sale:,.2f}")
    ...
    **What this app does**
    ...
    - computes profit / loss on sale for disposed assets using sale value
    - sets closing net block of disposed assets to zero
